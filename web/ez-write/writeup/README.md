...
MOCSCTF{USE_C7f_7rIcks_TO_Rce_!n_Re@LY_w0Rld}

## 解題步驟

1. 造訪首頁，發現程式碼的邏輯較為簡單，這確實是一個CMS裡真實的程式碼，被移植過來了：
...
2. 簡單來說，我們可以在一個被雙引號包裹了的地方寫入程式碼，不過我們不能使用雙引號、括號、分號和反引號，這裡用到兩個trick，首先，php裡如果被雙引號包圍，我們還是可以使用`${ php程式碼}`的方法執行被${}包裹的程式碼，不過這裡由於不允許使用括號和反引號，所以我們只能使用一些沒有括號的函數，例如include，這裡用到的另一個trick就是陸隊[The End Of LFI?](https://tttang.com/archive/1395/)裡提到的一個技巧，利用 PHP Base64 分析程式碼，這裡最後能打通的payload如下：
