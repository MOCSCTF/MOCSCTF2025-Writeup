...
if(version_compare(PHP_VERSION,'5.3.0','<'))  die('PHP的版本不能小于5.3');
//框架版本
...
function check_runtime() {
    ...
    mkdir(CACHE_PATH,0755,true);  // 模板缓存目录
    if(!is_dir(LOG_PATH))	mkdir(LOG_PATH,0755,true);    // 日志目录
    if(!is_dir(TEMP_PATH))  mkdir(TEMP_PATH,0755,true);	// 数据缓存目录
    if(!is_dir(DATA_PATH))	mkdir(DATA_PATH,0755,true);	// 数据文件目录
    return true;
}
