...
class ImageGd{
    ...
    public function open($imgname){
        ...
        }else if( 'bmp' == $this->info['type'] ){
        	$this->img = yd_imagecreatefrombmp($imgname);
        } else {
     ...
     * @throws Exception
     */
    public function saveAs($imgname, $other=array()){
    	if(empty($this->img)) throw new Exception('没有可以被保存的图像资源');
    	//原图像图像类型
    	$type = strtolower(yd_file_ext($imgname)); //目标图像格式
    	if(!in_array($type, array('jpeg','jpg','png','gif','bmp') )){
    		throw new Exception("不支持转化为{$type}格式图像");
    	}
    	$width= $this->info['width'];
    	$height = $this->info['height'];
		$dstImage = imagecreatetruecolor($width, $height);
		imagecopy($dstImage, $this->img, 0, 0, 0, 0, $width, $height);
    	//保存图像
    	if('gif' == $type){
    		imagegif($dstImage, $imgname);
    	}else if('bmp' == $type){
    		//7.0开始支持imagebmp函数
    		yd_imagebmp($dstImage, $imgname, 8, 0);
    	}else if('jpeg' == $type || 'jpg'==$type){
    		$JpegQuality = 100;
    		if( isset($other['JpegQuality']) && is_numeric($other['JpegQuality']) ){
    			$JpegQuality = $other['JpegQuality'];
    		}
    		//JPEG图像设置隔行扫描
    		imageinterlace($dstImage, true);
    		imagejpeg($dstImage, $imgname, $JpegQuality);
    	}elseif('png' == $type){
    		imagesavealpha($dstImage, true);
    		imagepng($dstImage, $imgname);
    	}
     ...
     */
    public function save($imgname, $type = null, $interlace = true, $other=array()){
        if(empty($this->img)) throw new Exception('没有可以被保存的图像资源');
        ...
        if('gif' == $type && !empty($this->gif)){
        	...
        	yd_imagebmp($this->img, $imgname,8, 0);
        }else if('jpeg' == $type){
        	$JpegQuality = 80; //不能默认为100，造成图片大小过大
        	if( isset($other['JpegQuality']) && is_numeric($other['JpegQuality']) ){
        		$JpegQuality = $other['JpegQuality'];
        	}
        	imagejpeg($this->img, $imgname, $JpegQuality);
        }elseif('png' == $type){
    ...
    public function text($text, $font, $size, $color = '#00000000', 
        ...
        //在7.1-7.3版，不加下面指令，就会有问题
        //putenv('GDFONTPATH=' . realpath('.')); //这条指令在7.3版无效
        ...
        switch ($locate) {
            ...
            default:
                /* 自定义文字坐标 */
                if(is_array($locate)){
                	if($locate[0]=='margin'){ //by wang 右边距，下边距
                		$x = ($locate[1] > $this->info['width']) ? $x : ($this->info['width']-$locate[1]);
                		$y = ($locate[2] > $this->info['height']) ? $y : ($this->info['height']-$locate[2]);
                	}else{
